...
| Optional historical-year trajectories | `test_historical_temperature_trajectories.py`, `test_historical_temperature_trajectories_sql.py` | Exactly 31 historical years align with all 25 Berlin-local lead hours; month boundaries, indexed source access, explicit opt-in, atomic refresh, and compact-snapshot isolation remain correct. |
| Static temperature dashboard | `test_temperature_dashboard.py` | Complete PLR geometry, summary, forecast, and historical exports; lazy per-PLR files; snapshot-only fallback; dependency-free browser assets; generated data excluded from Git. |
